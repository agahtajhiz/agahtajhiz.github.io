...
const RuleComponent = ( {
		...
		const processed = Object.keys( options ).map( ( groupKey ) => {
			return options[ groupKey ]
				.map( ( group ) => {
					...
					if ( ! actualTerms || actualTerms.length  {
						flatten.push( {
							label: term.name,
	...
	return (
			...
			<div classname="rule-set">
				<inlineselectgroup disabled false label="{" value="{" selectedroot onchange="{" onrootchange options="{" rootwithselect></inlineselectgroup>
				<selectcontrol disabled false aria-label="{" value="{" selectedcondition onchange="{" onconditionchange options="{" conditionoptions></selectcontrol>
				{ endConditions && (
					<virtualizedcombobox options="{" endoptionsstate value="{" selectedend as string onchange="{" onendchange dynamicquery="{" endroot setqueryopt="{" handlequerychange queryopt="{" query></virtualizedcombobox>
				) }
				<buttongroup classname="rule-actions">
					...
					{ ! ( isLast && isFirst ) && (
						<button label="{" __ rule icon="minus" iconsize="{" issecondary isdestructive classname="remove-rule is-small" disabled islast && isfirst onclick="{" removeruleset></button>
					) }
					{ isLast && (
						<button label="{" __ rule icon="{" plus iconsize="{" issecondary classname="add-rule is-small" disabled canaddmore onclick="{" addruleset></button>
					) }
				</buttongroup>
			</div>
		>
